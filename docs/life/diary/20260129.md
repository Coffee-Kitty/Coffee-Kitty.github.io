...
![因analyse缓存失效导致虽然tottime不高但cumtime特别高](image-31.png)
...
![patch本身主要改动的_fit函数，而该函数作为Cumtime极高的外部函数__call__的直接上层函数](image-32.png)
...
### human_judges: 
['xarray/core/dataset.py:interp', 
'xarray/core/dataset.py:_validate_interp_indexer',
'xarray/core/missing.py:_get_interpolator', 
'xarray/core/missing.py:interpolate_variable',
'xarray/core/missing.py:interp_func', 
'xarray/core/missing.py:_chunked_aware_interpnd',
'xarray/core/missing.py:decompose_interp', 
'xarray/core/missing.py:interp',
'xarray/core/missing.py:_interpnd',
'xarray/core/missing.py:_floatize_x',
'xarray/core/missing.py:_interp1d', 
'xarray/core/dataset.py:_validate_interp_indexers', 
'xarray/core/missing.py:_localize']

合理推出要改 Dataset.interp / missing.interpolate_variable / _interpnd / _localize，推断链是这样的：

SINK 是 dask.__bool__
→ 说明在比较时触发了 dask 的执行/化简（materialize）。

比较发生在 assert_identical，而测试名是 interpolate_chunk_advanced
→ 很大概率：比较的对象是 .interp 产生的结果。

.interp 自身看起来很快
→ 进一步加强判断：.interp 构建了一个“很贵的图”，执行成本被推迟到了 assert。

那么要让 dask.__bool__ 变便宜，根因不在 dask，而在 图是怎么被构建的
→ 图构建的关键控制点恰好就是：

Dataset.interp（indexers、sdims、dask_indexers 构建策略）

missing.interpolate_variable / _interpnd（gufunc 组织方式、broadcast/stack/localize）

_localize（是否误触发 compute）

注意：这里是“语义归因”，不是“调用栈归因”。
...
### human judge
['xarray/core/_aggregations.py:cumprod',
 'xarray/core/groupby.py:GroupBy', 
 'xarray/core/_aggregations.py:cumsum', 
 'xarray/core/_aggregations.py:any',
  'xarray/core/_aggregations.py:sum', 'xarray/core/groupby.py:_get_index_and_items', 'xarray/core/groupby.py:unique_value_groups', 'xarray/util/generate_aggregations.py:generate_example', 'xarray/core/groupby.py:first_items', 'xarray/core/_aggregations.py:max', 'xarray/core/groupby.py:_inverse_permutation_indices', 'xarray/core/groupby.py:_maybe_reorder', 'xarray/core/_aggregations.py:prod', 'xarray/core/groupby.py:_factorize_dummy', 'xarray/core/_aggregations.py:var', 'xarray/core/groupby.py:_factorize_bins', 'xarray/core/groupby.py:_flox_reduce', 'xarray/core/_aggregations.py:median', 'xarray/core/resample.py:_flox_reduce', 'xarray/core/groupby.py:_factorize_rest', 'xarray/core/groupby.py:copy', 'xarray/core/groupby.py:_concat_shortcut', 'xarray/core/groupby.py:_factorize_grouper', 'xarray/core/groupby.py:_get_output_index', 'xarray/core/resample_cftime.py:first_items', 'xarray/core/_aggregations.py:all', 'xarray/core/_aggregations.py:count', 'xarray/core/_aggregations.py:mean', 'xarray/core/groupby.py:_binary_op', 'xarray/core/_aggregations.py:min', 'xarray/core/groupby.py:__init__', 'xarray/core/groupby.py:_combine', 'xarray/core/nputils.py:inverse_permutation', 'xarray/core/groupby.py:_codes_to_groups', 'xarray/core/_aggregations.py:std']

![这个直接改C_rate最大的倒是也可以](image-34.png)
...
→ multipledispatch 选中 intersection_sets(ImageSet, Reals)
→ 为了判断“哪些像是实数”，大量走 as_real_imag
→ 期间频繁构造 re()/im()  **这里跑到了另一条分支上**
→ 触发 Function.__new__ → re.eval（也就是你 patch 的 complexes.py:eval）
```
...
但是patch确不是这里
### human judge
['sympy/functions/elementary/complexes.py:eval']
...
['sympy/matrices/matrices.py:_jblock_exponential']
真难找啊
...
### human patch
Identified 11 focus targets from patch: [
'testbed/bin/ask_update.py:get_all_known_matrix_facts',
'testbed/sympy/assumptions/ask_generated.py:get_all_known_number_facts', 
'testbed/bin/ask_update.py:get_known_facts_dict',
'testbed/sympy/assumptions/facts.py:get_number_facts',
'testbed/sympy/assumptions/facts.py:get_matrix_facts', 
'testbed/bin/ask_update.py:get_all_known_number_facts', 
'testbed/sympy/assumptions/satask.py:get_all_relevant_facts',
'testbed/sympy/assumptions/ask_generated.py:get_all_known_matrix_facts', 
'testbed/bin/ask_update.py:get_all_known_facts', 
'testbed/sympy/assumptions/ask_generated.py:get_all_known_facts',
'testbed/sympy/assumptions/facts.py:get_known_facts'
]

这个直接改了identify_removable_identity_matrices，反而加速了更多。。。。。。
